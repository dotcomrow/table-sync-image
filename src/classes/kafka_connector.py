...
class KafkaConnector:
    ...
    def delete_sink_cdc_connector(self, table_info: TableInfo):
        ...
        self.logger.logMessage(Logging.LogLevel.DEBUG, "Kafka Connect URL resolved for deletion", url=url)
        response = requests.delete(url)
        self.logger.logMessage(Logging.LogLevel.DEBUG, "Kafka Connect response for deletion", status_code=response.status_code, response_text=response.text)
    ...
    def get_cdc_stream_id(self, table_info):
        ...
        self.logger.logMessage(Logging.LogLevel.DEBUG, "Master addresses resolved", master_addresses=master_addrs)
        if not master_addrs:
        ...
        try:
            ...
            self.logger.logMessage(Logging.LogLevel.DEBUG, "yb-admin list_change_data_streams output", output=out)
        ...
        self.logger.logMessage(Logging.LogLevel.WARNING, "No CDC stream ID found or created")
        return None
    ...
    def check_connector_exists(self, table_info: TableInfo) -> bool:
        ...
        self.logger.logMessage(Logging.LogLevel.DEBUG, "Kafka Connect status URL", url=url)
        try:
    ...
    def create_sink_connector(self, table_info: TableInfo):
        ...
        try:
            ...
            self.logger.logMessage(Logging.LogLevel.ERROR, "Failed to load project_id from GCP key file", error=str(e))
            raise
